...
import blog_posts from "../src/data/blog-posts"; // Updated from './data/blog-posts'
...
const BlogPost = () => {
  ...
  const getLatestPost = (posts) =>
    posts?.slice().sort((a, b) => new Date(b.date) - new Date(a.date))[0];
  ...
  const toggleTag = (index) => {
    ...
    router.push(
      {
        pathname: router.pathname,
        query: { ...router.query, tags: JSON.stringify(newActiveTags) },
      },
      undefined,
      { shallow: true }
    );
  ...
  const displayedPosts = blog_posts.slice(
    (page - 1) * postsPerPage,
    page * postsPerPage
  );
    ...
    <div className="mt-24">
      ...
      <div className="flex flex-col items-center justify-center w-full gap-10 p-4 md:p-12">
        <div
          ...
          style={{
            boxShadow: "12px 12px 0px 0px #1F1F1F",
        ...
        >
          ...
          <div className="flex flex-col md:w-[60%] w-full gap-6 p-6">
            ...
            <div className="flex flex-col justify-between w-full gap-2 md:items-center md:flex-row">
              ...
              <div className="inline-flex items-center justify-center gap-4">
                {latestPost.tags.map((tag, index) => (
                  <div
                    key={index}
                    className="p-1.5 px-3 rounded-[5px] items-center justify-center inline-flex bg-[#FFECE0] text-[#FF6200] uppercase text-xs min-w-[60px]"
                  >
                    <h5>{tag}</h5>
                  </div>
                ))}
            ...
            <div className="inline-flex flex-col items-start justify-between gap-2">
              <h1 className="md:text-5xl heading text-3xl uppercase text-[#FF6200]">
                {latestPost.heading}
              </h1>
              <p className="text-justify para text-[#1F1F1F]/80 md:text-sm text-xs">
                {latestPost.content[0].desc}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-center w-full gap-16 m-auto mt-8 md:max-w-2/3">
        ...
        >
          <path
            opacity="0.4"
            d="M21.5406 17.0391L31.6406 24.6191V35.8391C31.6406 37.7591 29.3206 38.7191 27.9606 37.3591L17.6006 26.9991C15.9406 25.3391 15.9406 22.6391 17.6006 20.9791L21.5406 17.0391Z"
            fill="#171717"
            fillOpacity="0.3"
          />
          <path
            d="M31.6406 12.1599V24.6199L21.5406 17.0399L27.9606 10.6199C29.3206 9.27985 31.6406 10.2399 31.6406 12.1599Z"
            fill="#171717"
            fillOpacity="0.3"
          />
        </svg>
        ...
        {[...Array(totalPages)].map((_, index) => (
          <div
            ...
            className={`inline-flex items-center justify-center size-16 cursor-pointer ${
              page === index + 1
                ? "text-[#FF6200] rounded-xl bg-[#FF6200]/10"
                : "text-[#828282]"
            }`}
        ...
        >
          <path
            opacity="0.4"
            d="M26.4594 17.0391L16.3594 24.6191V35.8391C16.3594 37.7591 18.6794 38.7191 20.0394 37.3591L30.3994 26.9991C32.0594 25.3391 32.0594 22.6391 30.3994 20.9791L26.4594 17.0391Z"
            fill="#171717"
          />
          <path
            d="M16.3594 12.1599V24.6199L26.4594 17.0399L20.0394 10.6199C18.6794 9.27985 16.3594 10.2399 16.3594 12.1599Z"
            fill="#171717"
          />
        </svg>
      </div>
